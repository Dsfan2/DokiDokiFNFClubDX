...
class ChartingState extends MusicBeatState
{
	...
	[
		...
		['Buffsuki Punch', "FALCON PAWNCH!!"],
		// Camera
		['Add Camera Zoom', "Used on MILD on that one \"hard\" part\nValue 1: Camera zoom add (Default: 0.015)\nValue 2: UI zoom add (Default: 0.03)\nLeave the values blank if you want to use Default.\nNOTE: The camHUD value will not work if DS Filter is on!"],
		['Camera Follow Pos', "Value 1: X\nValue 2: Y\n\nThe camera won't change the follow point\nafter using this, for getting it back\nto normal, leave both values blank."],
		['Adv. Camera Follow', "Value 1: Character to focus on (bf, dad, gf)\nValue 2: Camera Speed (Default: camera_speed for stage JSON)\n\nThe camera won't change the follow point\nafter using this, for getting it back\nto normal, leave both values blank."],
		['Screen Shake', "Value 1: Camera shake\nValue 2: HUD shake\n\nEvery value works as the following example: \"1, 0.05\".\nThe first number (1) is the duration.\nThe second number (0.05) is the intensity."],
		['Epic Cam Beat', "Add Camera Zoom but cool\nValue 1: 1 = On, 0 = Off (Default: 0)"],
		['DefaultCamZoom Change', "Zoooooooom\nValue 1: New value for defaultCamZoom (Default: camZoom for stage JSON)\nValue 2: Duration (0 for instant)"],
		['Camera Bounce', "From Seek's Cool Deltarune Mod\nValue 1: 1 = On, 0 = Off (Default: 0)"],
		// Changers
		...
		['Stage Morph', "lol smash reference\nValue 1: Stage Name"],
		['Turn Pixel', "Used in Monika's Songs in DOKI DOKI difficulty.\nValue 1 turns it on and off\n(1 is on, 0 is off, default is 0)."],
		['Turn Classic', "Used in Poem Panic DOKI DOKI Remix.\nValue 1 turns it on and off\n(1 is on, 0 is off, default is 0)."],
		['Heath Drain', "Uh-Oh\nValue 1: 1 = On, 0 = Off (Default: 0)"],
		// Mechanics
		['Warning', "Shows Warning Sign.\nValues are meaningless"],
		['Double Warning', "Shows the Double Warning Sign.\nValues are meaningless again"],
		...
		['Popup Appear', "Just Monika\nValue 2: Duration (Must be greater than 0) (Default value is 1)"],
		['Distractions', "Shows Monika blocking the screen\nValue 1: The Sprite (1, 2, 3) (0 to remove)"],
		['Front Monika', "Used ONLY in Poems Are Forever\nValue 1: Opacity (Default: 0)"],
		// Hud
		['Show End Screen', "RIP Sayori\nValue 1: Angle (Default: 0)"],
		['Static Fade', "Fades the TV Static sprite.\nValue 1: Opacity (0 min, 1 max)\nValue 2: Fade duration (0 for instant)"],
		['Glitch Effect', "Ahahaha~\nValue 1: Glitch Type (1, 2, 3) (Leave it blank for random)\nValue 2: Effect duration (Must be greater than 0)"],
		['Black Screen Fade', "Fades a black screen.\nValue 1: Opacity (0 min, 1 max)\nValue 2: Fade duration (0 for instant)"],
		['Vingette Appear', "Value 1: 1 = Appear, 0 = Disappear (Default: 0)"],
		['Black Bar Thing', "Value 1: 1 = On, 0 = Off (Default: 0)"],
		['Hud Fade', "Fades the camHUD\nValue 1: Opacity (0 min, 1 max)\nValue 2: Fade duration (0 for instant)"],
		['White Screen Fade', "Fades a white screen, but now it's white :O\nValue 1: Opacity (0 min, 1 max)\nValue 2: Fade duration (0 for instant)"],
		['Hud Angle', "SPEEEEEEN\nValue 1: Angle (Default: 0)\nValue 2: Tween Duration (0 for instant)"],
		['Down on Up', "Swaps Downscroll & Upscroll\nValue 1: 1= On, 0 = Off (Default: 0"],
		['Screen Flash', "Value 1: 1 = White, 2 = Black, 3 = Static\n(Default: 1)"],
		// Stage Elements
		['Spotlight', "Used only in DOKI DOKI Remixes,\nValue 1: 0/1 = OFF/ON,\n2 = Target Opponent\n3 = Target BF\n4 = Target Both\nDefault: 0\n \nValue 2: The Smoke (0 = OFF, 1 = ON)"],
		['Glow Effect', "Used only in DOKI DOKI Remixes\nValue 1: 0/1/2 = OFF/ON/Reset Gradient\n \nWho's laughing now ShadowMario?"],
		['Binary Effect', "Only used in Spaceroom X Stage\nValue 1: 1 = On, 0 = Off (Default: 0)"],
		['Change Boxing Screen', "Used only in the Boxing Ring Stage\nValue 1: Screen Display (1, 2, 3, 4, 5, 6)\n(Default: 5)"],
		['Back Chars Invisible', "Turns the listed character invisible.\nUsed only in the Clubroom Stages\nValue 1: Monika, Sayori, Yuri, or Natsuki"],
		// Misc.
		['Play Sound', "Plays a sound file.\nValue 1: The sound file"],
		['Song Blurb', "Shows the song blurb"]
	];
	...
	**/
	...
	function addSongUI():Void
	{
		...
		{
			PlayState.SONG = Song.parseJSONtrash(FlxG.save.data.autosave);
		...
		var loadEventJson:FlxButton = new FlxButton(loadAutosaveBtn.x, loadAutosaveBtn.y + 30, 'Load Events', function()
		{
			...
			if (CoolUtil.difficultyString() == "DOKI DOKI")
			{
				...
				var file:String = Paths.json('charts/' + songName + '/events-doki');
				#if sys
				if (#if MODS_ALLOWED FileSystem.exists(Paths.modsJson('charts/' + songName + '/events-doki')) || #end FileSystem.exists(file))
				#else
				if (OpenFlAssets.exists(file))
				#end
				{
					clearEvents();
					var events:SwagSong = Song.loadFromJson('events-doki', songName);
		...
		var directories:Array<String> = [Paths.getPreloadPath('characters/bf/'), Paths.getPreloadPath('characters/gf/'), Paths.getPreloadPath('characters/jr/'), Paths.getPreloadPath('characters/misc/'), 
										Paths.getPreloadPath('characters/monika/'), Paths.getPreloadPath('characters/natsuki/'), Paths.getPreloadPath('characters/sayori/'), Paths.getPreloadPath('characters/yuri/')];
		/*#if MODS_ALLOWED
	...
	function loadSong():Void
	{
		...
		var file:Dynamic = Paths.opponentvoices(currentSongName);
		var file2:Dynamic = Paths.playervoices(currentSongName);
		vocals = new FlxSound();
	...
	{
		while (bulltrashUI.members.length > 0)
		{
			bulltrashUI.remove(bulltrashUI.members[0], true);
		}

		// general trash
		var title:FlxText = new FlxText(UI_box.x + 20, UI_box.y + 20, 0);
		bulltrashUI.add(title);
	}
		...
		if (id == FlxUICheckBox.CLICK_EVENT)
		{
			...
			switch (label)
			{
				...
				case 'Change BPM':
					...
					FlxG.log.add('changed bpm trash');
				case "Alt Animation":
	...
	override function update(elapsed:Float)
	{
		...
		if (!blockInput)
		{
			...
			if (FlxG.keys.justPressed.ENTER)
			{
				...
				if(playervox != null) playervox.stop();
			...
			if (FlxG.keys.justPressed.BACKSPACE) {
				...
				openSubState(new CustomFadeTransition(0.4, false, new MasterEditorMenu()));
			...
			if (FlxG.keys.justPressed.SPACE)
			{
				if (FlxG.sound.music.playing)
				{
					...
					if(playervox != null) playervox.pause();
				}
				else
				{
					if(vocals != null) {
					...
					if(playervox != null) {
						playervox.play();
						playervox.pause();
						playervox.time = FlxG.sound.music.time;
						playervox.play();
					}
			...
			if (FlxG.mouse.wheel != 0)
			{
				...
				if (!mouseQuant)
				...
				else
					{
						...
						if (FlxG.mouse.wheel > 0)
						{
							var crud:Float = CoolUtil.quantize(beat, snap) - increase;
							FlxG.sound.music.time = Conductor.beatToSeconds(crud);
						}else{
							var crud:Float = CoolUtil.quantize(beat, snap) + increase;
							FlxG.sound.music.time = Conductor.beatToSeconds(crud);
						}
			...
			if(!vortex){
				if (FlxG.keys.justPressed.UP || FlxG.keys.justPressed.DOWN  )
				{
					...
					{
						var crud:Float = CoolUtil.quantize(beat, snap) - increase; //(Math.floor((beat+snap) / snap) * snap);
						FlxG.sound.music.time = Conductor.beatToSeconds(crud);
					}else{
						var crud:Float = CoolUtil.quantize(beat, snap) + increase; //(Math.floor((beat+snap) / snap) * snap);
						FlxG.sound.music.time = Conductor.beatToSeconds(crud);
			...
			if(vortex && !blockInput){
				...
				if (FlxG.keys.justPressed.UP || FlxG.keys.justPressed.DOWN  )
				{
					...
					{
						var crud:Float = CoolUtil.quantize(beat, snap) - increase;
						feces = Conductor.beatToSeconds(crud);
					}else{
						var crud:Float = CoolUtil.quantize(beat, snap) + increase; //(Math.floor((beat+snap) / snap) * snap);
						feces = Conductor.beatToSeconds(crud);
					}
	...
	function resetSection(songBeginning:Bool = false):Void
	{
		...
		FlxG.sound.music.pause();
		// Basically old trash from changeSection???
	...
	function loadHealthIconFromCharacter(char:String) {
		var charDir:String = 'misc/';
		if (char.contains('bf'))
			charDir = 'bf/';
		if (char.contains('gf') || char.contains('speakers'))
			charDir = 'gf/';
		if (char.contains('jr'))
			charDir = 'jr/';
		if (char.contains('moni'))
			charDir = 'monika/';
		if (char.contains('nat') || char.contains('glitchsuki') || char.contains('buffsuki'))
			charDir = 'natsuki/';
		if (char.contains('sayo') || char.contains('sxyori'))
			charDir = 'sayori/';
		if (char.contains('yuri') || char.contains('yurdere'))
			charDir = 'yuri/';
		var characterPath:String = 'characters/' + charDir + char + '.json';
		#if MODS_ALLOWED
	...
	{
		//trashty null fix, i fucking hate it when this happens
	...
	private function saveEvents()
	{
		...
		if ((data != null) && (data.length > 0))
		{
			...
			_file.addEventListener(IOErrorEvent.IO_ERROR, onSaveError);
			if (CoolUtil.difficultyString() == "DOKI DOKI")
				_file.save(data.trim(), "events-doki.json");
			else
