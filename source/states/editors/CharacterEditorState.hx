...
class CharacterEditorState extends MusicBeatState
{
	...
	override function create()
	{
		...
		healthBar = new HealthBar(char.healthIcon, char.healthIcon, false, true);
		add(healthBar);
	...
	function reloadBGs() {
		...
		if(onPixelBG) {
			...
			var pixelBg:BGSprite = new BGSprite('stages/club/bg_pixel', -372, -145, 1.0, 1.0);
			pixelBg.setGraphicSize(Std.int(pixelBg.width * 6.6));
			pixelBg.updateHitbox();
			pixelBg.antialiasing = false;
			bgLayer.add(pixelBg);

			changeBGbutton.text = "Regular BG";
		} else {
			var bg:BGSprite = new BGSprite('stages/club/bg', -372, -147, 1.0, 1.0);
	...
	var TemplateCharacter:String = '{
			...
			"image": "characters/Blank_Nothing",
			"position": [
				0,
				0
			],
	...
	function addCharacterUI() {
		...
		var decideIconColor:FlxButton = new FlxButton(reloadImage.x, reloadImage.y + 30, "Get Icon Color", function()
			{
				var coolColor = FlxColor.fromInt(CoolUtil.dominantColor(healthBar.iconP2));
		...
		healthIconInputText = new FlxUIInputText(15, imageInputText.y + 35, 75, healthBar.iconP2.getCharacter(), 8);
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>) {
		if(id == FlxUIInputText.CHANGE_EVENT && (sender is FlxUIInputText)) {
			if(sender == healthIconInputText) {
				...
				healthBar.changeIcon(2, healthIconInputText.text);
			...
			else if(sender == healthColorStepperR)
			{
				...
				healthBar.reloadHealthBarColors(char.healthColorArray, char.healthColorArray);
