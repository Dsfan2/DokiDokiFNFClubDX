...
class NoteOffsetState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		boyfriend = new Character(770, 100, charact, true);
		...
		rating = new FlxSprite().loadGraphic(Paths.image('hudFolders/defaultUI/sick'));
		...
		{
			var numScore:FlxSprite = new FlxSprite(43 * daLoop).loadGraphic(Paths.image('hudFolders/defaultUI/num' + i));
		...
		timeTxt.setFormat(Paths.font("doki.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		timeBarBG = new FlxSprite(0, timeTxt.y + 8).loadGraphic(Paths.image('bars/timeBar'));
		...
		changeModeText.setFormat(Paths.font("doki.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		changeModeText.scrollFactor.set();
		changeModeText.borderSize = 1.5;
		changeModeText.cameras = [camHUD];
		add(changeModeText);
		updateMode();

		Conductor.changeBPM(128.0);
		CoolUtil.playMusic('PoeticOffsets');
	...
	override public function update(elapsed:Float)
	{
		...
		if(controls.BACK)
		{
			...
			openSubState(new CustomFadeTransition(0.4, false, new OptionsState()));
			//MusicBeatState.switchState(new options.OptionsState());
			if (OptionsState.onPlayState)
			{
				CoolUtil.playMusic(PauseSubState.pausesongname);
				//FlxG.sound.playMusic(Paths.music(PauseSubState.pausesongname));
			}
			else
	...
	function createTexts()
	{
		for (i in 0...4)
		{
			...
			text.setFormat(Paths.font("doki.ttf"), 24, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
