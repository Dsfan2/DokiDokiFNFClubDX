...
class GameplayChangersSubstate extends MusicBeatSubstate
{
	...
	public function new()
	{
		...
		var bg:DDLCBorderBG = new DDLCBorderBG(Paths.image('mainmenu/rgbBg'), -40, -40);
		add(bg);

		var border:DDLCBorder = new DDLCBorder(false);
		add(border);

		var titleText:FlxText = new FlxText(50, 50, 0, 'Gameplay Changers', 32);
		...
		for (i in 0...optionsArray.length)
		{
			...
			var optionText:FlxText = new FlxText(xx, yy, 0, optionsArray[i].name, 28);
			optionText.setFormat(Paths.font("dokiUI.ttf"), 28, FlxColor.WHITE, FlxTextAlign.RIGHT);
			optionText.setBorderStyle(OUTLINE, 0xFFBB5599, 4);
			optionText.ID = i;
			...
			if(optionsArray[i].type == 'bool') {
				...
				var valueText:AttachedDokiText = new AttachedDokiText('' + optionsArray[i].getValue(), optionText.width + 40, true);
	...
	override function update(elapsed:Float)
	{
		...
		grpTexts.forEach(function(txt:AttachedDokiText)
		{
			if (txt.ID == curSelected)
				txt.isSelected = true;
			else
